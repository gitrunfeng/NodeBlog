...
//友链首页
router.get('/yl',function(req,res){
    let page = Number(req.query.page || 1);
    let limit = 10;
    let pages=0;
    Yl.count().then(function(count){
        pages = Math.ceil(count/limit);
        page = Math.min(page,pages);
        page = Math.max(page,1)
        let skip = (page-1)*limit;
        ...
        * -1: 降序
        * */
       Yl.find().sort({_id:-1}).limit(limit).skip(skip).then(function(yls){
            res.render('admin/yl_index.html',{
                url:'/admin/yl',
                userInfo:req.userInfo,
                yls:yls,
                count:count,
...
//添加友链
router.get('/yl/add',function(req,res){
    res.render('admin/yl_add.html',{
        userInfo: req.userInfo,
    })
})
//添加友链保存
router.post('/yl/add', function(req,res){
...
//友链修改
router.get('/yl/edit',function(req,res){
    //获取要修改的分类的信息，并且用表单的形式展现出来
   let id = req.query.id || '';
   // console.log(id);
   //获取要修改的分类信息
...
// 友链保存修改页面
router.post('/yl/edit',function(req,res){
    //获取要修改的分类的信息，并且用表单的形式展现出来
    let id = req.query.id || '';
    //获取post提交过来的名称
    var title = req.body.title || '';
    var ylUrl = req.body.ylUrl || '';
    //获取要修改的分类信息
    Yl.findOne({
        _id:id
    }).then(function(yl){
...
//友链删除
router.get('/yl/delete',function(req,res){
    //获取要删除的分类的id
    let id = req.query.id || '';
    Yl.remove({
        _id:id
